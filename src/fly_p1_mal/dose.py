...
from fly_p1_mal.assay import AssayConfig, run_condition, write_run
from fly_p1_mal.physics import DT
from fly_p1_mal.subject import I_DA1, I_P1, load_W, load_slice
from malforge.gate import require_assay_order, require_engine
...
def run_da1_dose(cfg: AssayConfig, weights: np.ndarray | None = None) -> dict:
    ...
    spec = load_slice()
    cva = spec.get("cva_term") or {}
    return {
        "schema": "fly_p1_mal.p1_da1_dose.v1",
        "condition": "3d",
        "pin": True,
        "question": "On the 3d pin, at what W[P1, DA1] does mean P1 cross zero?",
        "honesty": (
            "Compressed cVA term is hop-1 DA1 plus hop-1 mAL GABA on the parent "
            "LC10a scale. Parent fly_p1_sign @45aa064 W_crit = -1.539. This default "
            "W is that folded term, not a hop-3 extract."
        ),
        "n_agents": 2,
        "seed": cfg.seed,
        "steps": cfg.steps,
        ...
        "cva_term": cva,
        "through_wcrit": bool(cva.get("through_wcrit")),
