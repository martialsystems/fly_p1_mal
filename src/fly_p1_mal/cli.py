...
from fly_p1_mal.assay import AssayConfig, run_assay, write_run
from fly_p1_mal.dose import run_da1_dose
from fly_p1_mal.claims import require_clean
from fly_p1_mal.paths import LOGS, REPO
from malforge.gate import LawBlockedError, require_assay_order, require_readme_clean

BANNER = (
    "Two agents. Hop-1 GABA from annotated mAL folded into W[P1, DA1] "
    "on the parent LC10a scale. Frozen object. Wiring stays."
)

LOCK_NAME = "p1_mal_s1.json"


def _parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="fly-p1-mal", description=BANNER)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"through_wcrit={(result.get('cva_term') or {}).get('through_wcrit')} "
